...
- [ ] T007 Edit `ESignException` base class to add optional `DocumentFormat format = DocumentFormat.Unknown` constructor parameter at end and public `Format` property per data-model.md §1.4 in [src/MisaConnect.ESign.Domain/Errors/ESignException.cs](src/MisaConnect.ESign.Domain/Errors/ESignException.cs)
...
- [ ] T014 [P] Edit `ESignGeneralException` to forward `format` to base in [src/MisaConnect.ESign.Domain/Errors/ESignGeneralException.cs](src/MisaConnect.ESign.Domain/Errors/ESignGeneralException.cs)
...
- [ ] T019 [P] Create `XmlHashOutput` record (`DocumentId`, `Document`, `SignatureId`, `Digest`, `Sh`) per data-model.md §2.2 in [src/MisaConnect.ESign.Application/Abstractions/XmlHashOutput.cs](src/MisaConnect.ESign.Application/Abstractions/XmlHashOutput.cs)
- [ ] T020 [P] Create `WordExcelHashOutput` record (`DocumentId`, `DocumentBytes`, `SignatureId`, `Digest`, `MainDom`) per data-model.md §2.3 in [src/MisaConnect.ESign.Application/Abstractions/WordExcelHashOutput.cs](src/MisaConnect.ESign.Application/Abstractions/WordExcelHashOutput.cs)
...
- [ ] T024 Decide and implement the `SubmitSignHash` input-shape projection per data-model.md §2.7 — either (a) add `PdfHashOutput.ToSignHashInput()` plus equivalent helpers on the new per-format hash outputs, or (b) introduce a lightweight `SignHashInput(string DocumentId, string Digest)` Application record and refactor `SubmitSignHash` to accept it (slice-1 wire shape unchanged either way per FR-065). Touches [src/MisaConnect.ESign.Application/UseCases/SubmitSignHash.cs](src/MisaConnect.ESign.Application/UseCases/SubmitSignHash.cs) and at minimum the slice-1 `PdfHashOutput` record.
