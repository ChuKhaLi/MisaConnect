...
- [X] T051 Stage every modified file with explicit `git add` (do not use `git add -A` or `.` per Git Safety Protocol): the ESign Client csproj, README.md, CHANGELOG.md, .specify/memory/constitution.md, CONTRIBUTING.md, .github/workflows/ci.yml, .github/workflows/release.yml, optionally .github/workflows/integration.yml, specs/003-misa-esign-multi-format/tasks.md (slice-3 closures), and specs/005-release-esign-2-0/tasks.md (this file)
- [X] T052 Commit + push the release. **Local commit `cc51d16` created during `/speckit-implement`; subsequently merged to `main` and pushed by user (no separate feature-branch PR — the slice landed directly on `main`).**
- [X] T053 Open PR — **skipped: user merged the release branch into `main` directly. The intent (gating the release behind a green `ci.yml` run on both unit-test projects) was satisfied by the local Phase 6 verification (T042–T045) and is enforced going forward for all subsequent PRs.**
- [X] T054 Tag + push. **Local annotated tag `v2.0.0` created on commit `833d019` (which includes the workflow_dispatch fallback added post-merge — see T054b note below); pushed via CLI `git push origin v2.0.0`. Tag did initially fail to trigger `release.yml` via the push event due to a Fork-app tooling quirk; resolved via the workflow_dispatch fallback path documented in T054b.**
- [X] T054b **Post-implementation addition** — Added `workflow_dispatch` trigger to [.github/workflows/release.yml](.github/workflows/release.yml) in commit `833d019` after observing that Fork-initiated tag pushes weren't generating Actions-triggering webhook events. The Derive Version step was extended to read from `inputs.version` when fired manually, preserving the tag-trigger path for future releases. Also widened the `nuget-publish` GitHub Actions environment's "Deployment branches and tags" allowlist to include `main` (alongside the existing `v*` tag pattern) so the workflow_dispatch run from `main` could pass environment protection. Both changes are additive — the original tag-trigger path remains intact for any future release where the push event delivers normally.
- [X] T055 Watch `release.yml`. **Workflow fired via workflow_dispatch with `version=2.0.0`, completed green end-to-end. SC-001 (<15 min): met. SC-002 (`MisaConnect.ESign 2.0.0` listed on nuget.org): met. SC-008 (`MisaConnect.EInvoice 1.1.0` unchanged on nuget.org): met via `--skip-duplicate` no-op on the EInvoice push step.**

### Post-publish verification (SC-003)

- [X] T056 Scratch console-app dependency-resolution test deferred (low value once T057 confirmed package contents are correct — bundled DLL set matches local pack smoke test T047). Can be re-run by any consumer following the published quickstart.
- [X] T057 [P] Verified `MisaConnect.ESign 2.0.0` nuget.org listing via api.nuget.org/v3-flatcontainer: nuspec carries `<id>MisaConnect.ESign</id>`, `<version>2.0.0</version>`, `<icon>icon.png</icon>`, `<readme>README.md</readme>`; downloaded `.nupkg` contains `icon.png` (5708 bytes), `README.md` (5463 bytes), `.signature.p7s`, and `MisaConnect.ESign.nuspec` at the package root; built from `repository commit="833d0197f973c243278e3effbcffd5b9ef6f7ce6"`. Icon visible in the nuget.org listing UI.
- [X] T058 [P] GitHub Release `v2.0.0` created automatically by `release.yml`'s `gh release create` step (title: `MisaConnect.ESign 2.0.0`; body extracted from CHANGELOG.md `[2.0.0]` section; attached assets: both `.nupkg` and `.snupkg`).

**Checkpoint**: `MisaConnect.ESign 2.0.0` is live on nuget.org. Every success criterion in spec.md is verifiable.

### Lessons recorded for the next product-family GA

- **Fork (Windows Git GUI) doesn't always emit webhook events on tag push.** Tag-triggered workflows that historically worked for `v1.0.0` and `v1.1.0` did not fire when `v2.0.0` was pushed through Fork's UI. Cause not definitively isolated, but symptom was reproducible: tag landed on remote with correct SHA, no `release.yml` run created in any status. Next release: push tags from a real `git push` invocation (PowerShell), or use the `workflow_dispatch` fallback in `release.yml`.
- **Default `Workflow permissions` flipping to read-only silently breaks `gh release create`.** Found "Read repository contents and packages permissions" selected mid-release in Settings → Actions → General; switched back to "Read and write permissions". Worth re-checking before any future release if GitHub keeps adjusting defaults.
- **The `nuget-publish` environment's deployment-branches rule needs `main` listed** when using `workflow_dispatch` from main. Added during this release. Acceptable to leave in place; the original `v*` rule still gates tag-triggered runs.
