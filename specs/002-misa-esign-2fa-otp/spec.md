...
- **FR-040**: Every OTP-related exception (challenge surfaced, exchange rejected, resend rejected) MUST carry MISA's `errorCode` and the request correlation ID on dedicated properties accessible without inspecting the exception `Message` text. The raw vendor strings `devMsg` and `userMsg` MUST be carried only when the consumer has opted in via `MisaESignOptions.Errors.IncludeRawErrorMessage = true` — matching slice 1's secret-hygiene pattern and Constitution Principle VIII. When the flag is `false` (the default), the typed exception's synthesized `Message` text MUST NOT include MISA's raw `devMsg` or `userMsg` content. The resend operation's typed-failure result (FR-038) is unaffected by this flag and carries `RawCode`, `UserMsg`, and `DevMsg` directly on `OtpResendResult` regardless of the flag's state, because that result is the typed contract surface (not a logged or thrown vendor message).
