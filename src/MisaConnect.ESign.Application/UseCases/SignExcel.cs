...
internal sealed record BeginSignExcelCoreResult(AccessToken Token, Certificate Cert, WordExcelHashOutput Hash, SignTransaction Transaction);
...
public sealed class SignExcel
{
    ...
    internal async Task<BeginSignExcelCoreResult> BeginSignExcelCoreAsync(SignExcelWorkRequest request, CancellationToken ct)
    {
        ...
        return new BeginSignExcelCoreResult(token, cert, hash, transaction);
    }

    private async Task<SignExcelWorkResult> ExecuteSignAsync(SignExcelWorkRequest request, CancellationToken ct)
    {
        var core = await BeginSignExcelCoreAsync(request, ct).ConfigureAwait(false);
