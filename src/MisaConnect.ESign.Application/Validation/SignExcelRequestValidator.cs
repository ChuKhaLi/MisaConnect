...
public sealed class SignExcelRequestValidator
{
    private readonly ICorrelationIdAccessor _correlation;

    public SignExcelRequestValidator(ICorrelationIdAccessor correlation)
    {
        _correlation = correlation;
    }

    public void Validate(SignExcelWorkRequest request)
    {
        ArgumentNullException.ThrowIfNull(request);

        var errors = new List<string>();

        if (request.Excel is null || request.Excel.Length == 0)
        {
            errors.Add("Excel bytes must be non-empty.");
        }
        ...
        if (errors.Count > 0)
        {
            throw new ESignGeneralException(
                ...
                "InvalidSignExcelRequest",
                "SignExcelRequest failed validation: " + string.Join("; ", errors),
